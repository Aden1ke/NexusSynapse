...
def generate_ai_fix_instructions(code: str, issues: list) -> str:
    ...
    try:
        ...
        response = client.complete(
            messages=[
                SystemMessage(
                    content="You generate specific line-by-line coding fix instructions."
                ),
...
def get_ai_review_score(code: str, task: str) -> tuple:
    ...
    try:
        ...
        response = client.complete(
            messages=[
                SystemMessage(
                    content="You are a senior code reviewer. You provide JSON feedback with 'score', 'feedback', and 'instructions'."
                ),
                UserMessage(content=prompt),
            ],
            model=os.getenv("MODEL_DEPLOYMENT_NAME", "gpt-4o"),
                ...
                issues.append(
                    {
                        "type": "AZURE-SAFETY",
                        "severity": (
                            "HIGH" if category_result.severity > 4 else "MEDIUM"
                        ),
...
def scan_code(
    code: str,
    task: str = "No task description provided.",
    ...
    Returns:
        dict: {
            verdict:          "APPROVED", "REJECTED", or "PERMANENTLY_REJECTED",
    ...
    try:
        ...
        for issue in result["details"]["lint"]:
            ...
            if issue["type"] == "E":
                scanner_deductions += 10
            elif issue["type"] == "W":
                scanner_deductions += 5
        ...
        for issue in result["details"]["security"]:
            if "severity" in issue and issue.get("type") != "AZURE-SAFETY":
                ...
                if sev == "HIGH":
                    scanner_deductions += 20
                elif sev == "MEDIUM":
                    scanner_deductions += 10
                elif sev == "LOW":
                    scanner_deductions += 3
        ...
        if scanner_score < 50:
            ...
            result["summary"] = (
                f"❌ REJECTED: Scanner Gate failed ({scanner_score}/100).\n{fix_instructions}"
            )
            return result
        ...
        if ai_score < 60:
            ...
            result["approved"] = False
            result["summary"] = (
                f"❌ REJECTED: AI Review Gate failed ({ai_score}/100). Code does not solve the task correctly: {ai_feedback}"
            )
            return result
        ...
        result["summary"] = (
            f"✅ APPROVED. Gatekeeper score: {result['score']}/100. (AI: {ai_score}, Scanner: {scanner_score}, Safety: {safety_score})"
        )
...
"""
    ...
    print(
        f"Details : {len(r1['details']['lint'])} lint, {len(r1['details']['security'])} security"
    )
